...
RSpec.describe Bard::Backup::S3Dir do
  ...
  describe "#bucket_name" do
    it "is the first part of the path" do
      subject = described_class.new(path: "bard-backup/test", access_key_id:, secret_access_key:, region:)
      expect(subject.bucket_name).to eq "bard-backup"
    end

    it "is the whole path if no subdirectory" do
      subject = described_class.new(path: "bard-backup", access_key_id:, secret_access_key:, region:)
      expect(subject.bucket_name).to eq "bard-backup"
    end
  end
